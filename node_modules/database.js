...
module.exports.createCitation = function(loginId, sourceURL, sourceAuthor, sourceTitle, containerTitle, publisherTitle, sourcePublicationDate, callback) {
    query("INSERT INTO Citation (Login_id, sourceURL, sourceAuthor, sourceTitle, containerTitle, publisherTitle, sourcePublicationDate) VALUES (?, ?, ?, ?, ?, ?, ?);", [loginId, sourceURL, sourceAuthor, sourceTitle, containerTitle, publisherTitle, sourcePublicationDate], function(rows) {
        if (rows == null) {
...
module.exports.formatCitation = function(citationId, callback) {
    query("SELECT * FROM Citation WHERE id=?;", [citationId], function (rows) {
        if (rows == null) {
            ...
            citationText += "Accessed " + now.getDate() + " " + months[now.getMonth()] + ". " + now.getFullYear() + ".";
...
module.exports.formatCitations = function(loginId, limit, callback) {
    query("SELECT * FROM Citation" + (loginId ? " WHERE Login_id=?" : "") + " ORDER BY creationDate DESC LIMIT ?;", (loginId ? [loginId, limit] : [limit]), function(rows) {
        if (rows == null) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                var date = new Date(rows[i].creationDate);

                // http://ia.juniata.edu/citation/mla/mla-months.htm
                var months = ['Jan.', 'Feb.', 'Mar.', 'Apr.', 'May', 'June', 'July', 'Aug.', 'Sept.', 'Oct.', 'Nov.', 'Dec.'];

                citationText += "Accessed " + date.getDate() + " " + months[date.getMonth()] + " " + date.getFullYear() + ".";

                citations.push({url: rows[i].sourceUrl, text: citationText});
            }
