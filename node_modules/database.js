            ...
            if (rows == null) {
                ...
                query("SELECT id FROM Login WHERE name=?;", [name], function(rows) {
                    if (rows == null || rows.length != 1) {
                        ...
                        if (referralCode) {
                            query("UPDATE Referral SET recipientLogin_id=? WHERE code=? AND recipientLogin_id=NULL;", [rows[0].id, referralCode], function (rows) {
                                if (rows == null) {
...
module.exports.getLogin = function(loginId, callback) {
    query("SELECT Login.id, Login.name, Login.firstName, Login.lastName, Login.email, Login.priviledge, Login.creationDate, Login.upgradeEndDate, COUNT(Citation.id) as citationCount, COUNT(Referral.id) as referralCount, COUNT(Referral.recipientLogin_id) as referralRecipientCount FROM Login LEFT JOIN Referral on Login.id=Referral.Login_id LEFT JOIN Citation ON Login.id=Citation.Login_id WHERE Login.id=? GROUP BY Login.id;", [loginId], function(rows) {
        if (rows == null) {
            databaseError("Could not select row from Login.");
        } else if (rows.length != 1) {
            callback(null);
...
module.exports.getLogins = function(callback) {
    query("SELECT Login.id, Login.name, Login.firstName, Login.lastName, Login.email, Login.priviledge, Login.creationDate, Login.upgradeEndDate, COUNT(Citation.id) as citationCount, COUNT(Referral.id) as referralCount, COUNT(Referral.recipientLogin_id) as referralRecipientCount FROM Login LEFT JOIN Referral on Login.id=Referral.Login_id LEFT JOIN Citation ON Login.id=Citation.Login_id GROUP BY Login.id;", [], function(rows) {
        if (rows == null) {
