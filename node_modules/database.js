...
module.exports.getLogin = function(loginId, callback) {
    query("SELECT Login.id, Login.name, Login.firstName, Login.lastName, Login.email, Login.priviledge, Login.creationDate, Login.upgradeEndDate, COUNT(Citation.id) as citationCount FROM Login LEFT JOIN Citation ON Login.id=Citation.Login_id WHERE Login.id=? GROUP BY Login.id;", [loginId], function(rows) {
        if (rows == null) {
            databaseError("Could not select row from Login.");
        } else if (rows.length != 1) {
            callback(null);
...
module.exports.getLogins = function(callback) {
    query("SELECT Login.id, Login.name, Login.firstName, Login.lastName, Login.email, Login.priviledge, Login.creationDate, Login.upgradeEndDate, COUNT(Citation.id) as citationCount FROM Login LEFT JOIN Citation ON Login.id=Citation.Login_id GROUP BY Login.id;", [], function(rows) {
        if (rows == null) {
